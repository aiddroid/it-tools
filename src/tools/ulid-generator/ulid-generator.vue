...
<template>
  <div flex flex-col justify-center gap-2>
    <div flex items-center>
      <label w-75px> {{ t('tools.ulid-generator.label.quantity') }}:</label>
      <n-input-number v-model:value="amount" min="1" max="100" flex-1 />
    </div>

    <c-buttons-select v-model:value="format" :options="formats" :label="t('tools.ulid-generator.label.format')" label-width="75px" />
